...
const { process_multipart_data } = require('../../shared/process-multipart.js');
const UploadedFile = require('../../shared/uploaded-file.js');
...
class Request {
    ...
    binary() {
        // Resolve from cache if available
        if (this._body_binary) return Promise.resolve(this._body_binary);

        // Initialize the text promise if it does not exist
        this._binary_promise = new Promise((resolve) =>
            this._body_parser_get_received_data().then((raw) => {
                // Convert the Uint8Array buffer into a Buffer
                this._body_binary = Buffer.from(raw);

                // Resolve the text promise with the body text
                resolve(this._body_binary);
            }),
        );
    ...
    async processBody() {
        ...
        this._body_promise = new Promise(async (resolve, reject) => {
            let bodyData = undefined;

            if (contentType === 'application/json') {
                bodyData = await this.json();
            } else if (contentType === 'application/x-www-form-urlencoded') {
                bodyData = await this.urlencoded();
            } else if (contentType.includes('multipart/form-data')) {
                bodyData = await process_multipart_data(this);
            } else if (contentType === 'text/plain') {
                bodyData = { $body: await this.text() };
            } else if (contentType === 'text/html' || contentType === 'application/xml') {
                bodyData = { $body: (await this.buffer()).toString() };
            } else {
                bodyData = { $body: await this.buffer() };
            }
    ...
    bearerToken() {
        ...
        if (asArray?.length) return asArray[1];
        return undefined;
    }
    ...
    file(key) {
        const dataAtKey = this._body[key];
        if (Array.isArray(dataAtKey)) {
            for (const dataAtIndex of dataAtKey) {
                if (dataAtIndex instanceof UploadedFile) return dataAtKey;
            }
        } else if (dataAtKey instanceof UploadedFile) {
            return dataAtKey;
        }
